...
- The count in this gate counts sweep-enabled, adjudicated reviews only. It
  ...
  recall, variance, or cost claim at all. A zero denominator — ten counted pull requests whose external reviews
...
- Precision evidence is mandatory for every recall claim: the same comparison
  ...
  category-list version and effective sweep configuration as the comparison the
  claim is made over, so precision evidence produced under a since-revised
  category list does not back a recall claim. Category attribution
...
- Every other reachable combination leaves the tier unchanged. Fixture evidence
  ...
  stays Real-PR evidence (provisional); Real-PR evidence (measured) without a
  revision stays Real-PR evidence (measured). No combination demotes a tier
  except a revision of the category list at Real-PR evidence (measured), and no
  combination promotes one except the first recorded sweep-enabled real pull
  request review and the tenth counted pull request.
...
- [ ] AC15: Recorded evidence carries exactly one of the three evidence tier
      ...
      **mean of the per-run recalls** — one observation per run, each run's
      ...
      confirmed-clean reviews; the same zero denominator makes the
      ...
      claim is made over — together with its recorded regression result, because
...
| O6: Precision evidence | AC9, AC10 | Sweep-off and sweep-on unexpected findings are counted; sweep-on findings are attributed per category and sweep-off findings are reported as unattributed; a strict no-tolerance test decides regression; a clean pass stays clean; the runs record their category-list version and effective sweep configuration, so under a revised list the previous list's precision evidence cannot back a recall claim. |
