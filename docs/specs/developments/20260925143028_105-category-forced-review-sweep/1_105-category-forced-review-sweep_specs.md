...
- Real-pull-request effect of the sweep may not be claimed until at least ten
  ...
  cost both require the same number of runs on both configurations, at least
  two each, so an evidence set with a different run count per configuration
  carries no comparative recall, variance, or cost claim.)
...
- [ ] AC15: Recorded evidence carries exactly one of the three evidence tier
      ...
      **confirmed defects** recorded for those heads — a defect found by any
      ...
      configurations, at least two each, so an asymmetric evidence set where one
      configuration was repeated and the other was not supports no comparative
      recall claim at all — the comparative recall claim is stated over the
