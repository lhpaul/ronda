...
- The count in this gate counts sweep-enabled, adjudicated reviews only. It
  ...
  claim is stated over the mean of the per-run recalls, one observation per
  run, rather than a union or majority rule over them. A zero denominator — ten counted pull requests whose external reviews
...
- [ ] AC15: Recorded evidence carries exactly one of the three evidence tier
      ...
      **confirmed external defects** recorded for those heads — an adjudicated
      ...
      was run more than once on those heads, as the repeated-run variance
      evidence requires, the comparative recall claim is stated over the
      **mean of the per-run recalls** — one observation per run, each run's
      ...
      where each configuration was run more than once on those heads, the
