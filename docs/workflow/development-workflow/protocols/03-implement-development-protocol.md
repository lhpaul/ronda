...
- [ ] **Concurrent / parallel execution**: for scripts that write shared state (files, git objects, GitHub API rate-limited resources), does the harness include concurrent-invocation coverage (a test or explicit isolation rationale), or explicitly document why isolation makes this safe?
...
- [ ] **Isolating planted violation per new assertion**: for each **new assertion** this PR introduces in the harness, cite at least one **isolating** plant at a concrete file and line with both directions recorded — the assertion fails with the plant applied and passes once the plant is removed (or with a plant that does not target that assertion). One combined rollback that flips several assertions does **not** satisfy several new assertions unless isolating pairings exist for each assertion that stayed green under the combined edit. A plant that flips an assertion only because sibling fields or assertions changed in the same edit does not count. See `REVIEW.md` → Workflow Policy Review Checklist item 4 and `docs/best-practices/3-testing.md` → "Planted-Violation Proofs".
