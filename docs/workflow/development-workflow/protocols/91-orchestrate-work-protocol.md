...
Run this checklist for **every PR** via the dedicated script (exit codes in the table
above). Forward Step 7 `POST_CLEAN_*` / `LOCAL_AI_*` and Step 8 `CI_EVIDENCE` through
the environment, or supply them with `--evidence-file` / `--evidence-stdin` (one
`KEY=value` per line; `#` comments and blank lines allowed). Run `--help` on the script
...
EVIDENCE_FILE=step7-step8-evidence.env  # optional: saved POST_CLEAN_* / LOCAL_AI_* / CI_EVIDENCE lines

bash ./scripts/development-workflow/pr-label-readiness-checklist.sh "$PR_NUMBER" \
  --branch "$BRANCH" \
  ${EVIDENCE_FILE:+--evidence-file "$EVIDENCE_FILE"}
```

The script implements Checks 0 through 4. Do not copy-paste a duplicate inline
checklist block into this protocol — invoke `pr-label-readiness-checklist.sh` instead.
